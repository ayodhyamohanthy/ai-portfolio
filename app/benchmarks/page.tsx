import Link from 'next/link';import TopBoards from './TopBoards';import {snapshots,results,boards,guiBench,denseBoards,highlights} from '../benchData';import Reveal from '../Reveal';
...
<section className="sources-track" id="top-boards" data-rv>
<div className="track-head"><b>02</b><div><h2>Top leaderboards</h2><p>The Design Arena frame, mirrored: every arena and cut on the board - code, image, video, audio, slides, SVG and ASCII - with the Elo / win-rate toggle and the All Models / Open Weights / Routers filter, all working off live board data read 13 Sep 2026. Cuts with no public data feed link straight to the live board. Refreshes on the sweep.</p></div></div>
<TopBoards/>
</section>
...
<div className="track-head"><b>{String(di+3).padStart(2,'0')}</b><div><h2>{d.board}</h2><p>{d.category} · {d.what}</p></div></div>
...
<div className="track-head"><b>05</b><div><h2>{s.board}</h2><p>{s.category} · {s.what}</p></div></div>
...
<div className="track-head"><b>06</b><div><h2>{guiBench.title}</h2><p>{guiBench.what}</p></div></div>
...
<div className="track-head"><b>07</b><div><h2>Reported results</h2><p>Vendor-published benchmark numbers, kept with the claim and the link.</p></div></div>
...
<div className="track-head"><b>08</b><div><h2>Boards we watch</h2><p>Live leaderboards checked on the sweep. Snapshots above are re-read as they move.</p></div></div>
